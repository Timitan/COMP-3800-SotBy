...
async function testCourseCreation(driver, driver2, {number, subject, course, title, length, color}, {number2, subject2, course2, title2, length2, color2}) {
    console.log(number);
    // Fill out form data
    ...
    const elementList2 = await driver2.wait(until.elementsLocated(By.name(`${title + " " + number + " el"}`)), 10000);

    // Check if course info was correctly sent over to the other browser
    const elementList3 = await driver.wait(until.elementsLocated(By.name(`${title + " " + number + " el"}`)), 8000);
...
async function testCourses(driver, driver2, course, course2, name, name2) {
    ...
    // Create test courses
    //await testCourseCreation(driver, driver2, course, course2, name, name2);
